...
filename_vout_post = dev_path + "/res/dac_" + domen_post + "_vout_" + operation + "_" + "vol" + ".res"
...
results_vout_post = np.loadtxt(filename_vout_post, skiprows=1)
...
#
print (results_vout_post.shape)

cols = results_vout_post.shape[1]

x_vout_post = results_vout_post[:, 0]*1000
y_vout_post = results_vout_post[:, 1:cols]
label_curve_vout_post = ['Vout_post']
...
for curve in range(y_vout_post.shape[1]):
    label6 = label_curve_vout_post[curve]
    axs[4].plot (x_vout_post, y_vout_post[:, curve], label=label6)
